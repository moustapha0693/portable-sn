-- (Intermédiaire) CA "réalisé" = livrées, "en cours" = non livrées.
-- Superseded par 0024 : le chiffre d'affaires est retiré du tableau de bord.
...
begin
  ...
  select jsonb_build_object(
    'revenue_delivered',       coalesce((select sum(total) from public.orders where status = 'livree'), 0),
    'revenue_delivered_month', coalesce((select sum(total) from public.orders where status = 'livree' and created_at >= date_trunc('month', now())), 0),
    'revenue_pending',         coalesce((select sum(total) from public.orders where status in ('en_attente','confirmee','en_livraison')), 0),
    ...
    'sales_14d',       (select coalesce(jsonb_agg(jsonb_build_object('d', d, 'orders', o, 'revenue', r) order by d), '[]'::jsonb)
                          from (select gs::date d,
                                  ...
                                  (select coalesce(sum(total),0) from public.orders where created_at::date = gs::date and status = 'livree') r
